...
export const toModel = (hqdm: HQDMModel): Model => {
  ...
  const possibleWorld = hqdm.findByType(possible_world)[0]; // Assumes just one possible world
  if (possibleWorld) {
    m.name = hqdm.getIdentifications(possibleWorld, communityName)[0]?.id;
    m.description = hqdm.getDescriptions(possibleWorld, communityName)[0]?.id;
  }
  ...
  hqdm.findByType(ordinary_physical_object).forEach((i) => {
    ...
    const name = identifications[0]?.id ?? "No Name Found: " + i.id; // Assumes just one name
    ...
    const description = descriptions[0]?.id; // Assumes just one description
  ...
  hqdm.findByType(activity).forEach((a) => {
    ...
    const name = identifications[0]?.id ?? "No Name Found: " + a.id; // Assumes just one name
    ...
    participations.forEach((p) => {
      ...
      const participantRoles = hqdm.getRole(p);
      const participantRole =
        participantRoles.length > 0 ? participantRoles[0] : undefined;
